...
def update_ip_map(server, service, iphere, ipthere):
    ...
    if cluster_data_yaml_parsed:
        if server in cluster_data_yaml_parsed.keys():
            ...
            if service == 'web':
                ...
                mydict = {server: {'dnsmap': {iphere: ipthere}}}
...
if os.path.isfile(cluster_config_file):
    update_ip_map(server_key, service, ip_here, remote_ip)
else:
    if service == 'web':
        mydict = {server_key: {'ipmap': {ip_here: remote_ip}}}
    elif service == 'dns':
        mydict = {server_key: {'dnsmap': {ip_here: remote_ip}}}
