...
if form.getvalue('domain') and form.getvalue('backend'):
    ...
    if os.path.isfile(profileyaml):
        ...
        if os.path.isfile(backend_config_file) and os.path.isfile(profile_config_file):
            ...
            print('<HR>')
            print('<p style="background-color:LightGrey">Enable/Disable NAXSI Web Application Firewall: </p>')
            print('<input type="radio" name="naxsi" value="0" checked/> DISABLE')
            print('<input type="radio" name="naxsi" value="1" /> ENABLE')
            print('<HR>')
