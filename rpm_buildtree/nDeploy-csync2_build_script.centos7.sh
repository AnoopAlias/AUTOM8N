...
fpm -s dir -t rpm -C ../nDeploy-csync2-pkg-centos7 --vendor "PiServe Technologies" --version ${CSYNC2_VERSION} --iteration ${CSYNC2_RPM_ITER} -a $(arch) -m info@piserve.com -e --description "nDeploy custom csync2 package" --url http://piserve.com --conflicts csync2 -d librsync -d gnutls -d sqlite --after-install ../after_csync2_install --before-remove ../after_csync_uninstall --name csync2-nDeploy .
