...
##Vars
NGINX_VERSION="1.8.1"
NGINX_RPM_ITER="4.el6"
NPS_VERSION="1.10.33.4"
MY_RUBY_VERSION="2.3.0"
PASSENGER_VERSION="5.0.24"
...
mkdir -p var/run
fpm -s dir -t rpm -C ../nginx-pkg-64 --vendor "PiServe Technologies" --rpm-sign --version ${NGINX_VERSION} --iteration ${NGINX_RPM_ITER} -a $(arch) -m info@piserve.com -e --description "nDeploy custom nginx package" --url http://piserve.com --conflicts nginx -d zlib -d openssl -d pcre -d libcurl -d memcached --after-install ../after_nginx_install --before-remove ../after_nginx_uninstall --name nginx-nDeploy .
rsync -av nginx-nDeploy-* root@rpm.piserve.com:/home/rpmrepo/public_html/CentOS/6/x86_64/
