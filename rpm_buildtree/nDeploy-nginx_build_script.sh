...
for module in brotli geoip pagespeed passenger redis redis2 set_misc srcache_filter echo modsecurity testcookie_access
do
...
fpm -s dir -t rpm -C ../nginx-pkg --vendor "Anoop P Alias" --version ${NGINX_VERSION} --iteration ${NGINX_RPM_ITER} -a $(arch) -m anoopalias01@gmail.com --description "nDeploy custom nginx package" --url http://anoopalias.github.io/XtendWeb/ --conflicts nginx --conflicts openresty-nDeploy --conflicts nginx-nDeploy-module-naxsi --before-install ../before_ndeploy_install --after-install ../after_nginx_install --name nginx-nDeploy .
