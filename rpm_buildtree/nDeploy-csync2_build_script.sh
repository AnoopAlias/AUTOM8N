...
fpm -s dir -t rpm -C ../nDeploy-csync2-pkg --vendor "PiServe Technologies" --version ${CSYNC2_VERSION} --iteration ${CSYNC2_RPM_ITER} --rpm-sign -a $(arch) -m info@piserve.com -e --description "nDeploy custom csync2 package" --url http://piserve.com --conflicts csync2 -d xinetd -d librsync -d gnutls -d sqlite -d sqlite-devel --after-install ../after_csync2_install --before-remove ../after_csync2_uninstall --name csync2-nDeploy .
